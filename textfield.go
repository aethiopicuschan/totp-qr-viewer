...
func WithMultilines(multilines bool) TextFieldOption {
	return func(tf *TextField) {
		tf.multilines = multilines
	}
}
...
func NewTextField(bounds image.Rectangle, opt ...TextFieldOption) *TextField {
	t := &TextField{
		bounds:     bounds,
		multilines: false,
		color:      color.White,
	...
	if t.label != "" {
		labelHeight := int(fontFace.Metrics().HLineGap+fontFace.Metrics().HAscent+fontFace.Metrics().HDescent) / 2
		t.bounds.Max.Y += labelHeight
	}
	return t
}
...
func (t *TextField) cursorPos() (int, int) {
	...
	if t.label != "" {
		y += int(fontFace.Metrics().HLineGap+fontFace.Metrics().HAscent+fontFace.Metrics().HDescent) / 2
	}
	return x, y
}

func (t *TextField) Draw(screen *ebiten.Image) {
	vector.DrawFilledRect(screen, float32(t.bounds.Min.X), float32(t.bounds.Min.Y), float32(t.bounds.Dx()), float32(t.bounds.Dy()), t.color, false)
	...
	if t.label != "" {
		op := &text.DrawOptions{}
		op.GeoM.Translate(float64(t.bounds.Min.X+px), float64(t.bounds.Min.Y)-fontFace.Metrics().HLineGap-2) // 上部に少しずらす
		op.ColorScale.ScaleWithColor(color.Black)
		text.Draw(screen, t.label, fontFace, op)
	}
	...
	if t.label != "" {
		ty += int(fontFace.Metrics().HLineGap+fontFace.Metrics().HAscent+fontFace.Metrics().HDescent) / 2
	}
	op := &text.DrawOptions{}
